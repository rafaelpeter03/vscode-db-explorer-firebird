...
export function getViewsQuery(maxViewCount: number): string {
  if (maxViewCount !== 0) {
    return `SELECT FIRST ${Math.abs(maxViewCount)} RDB$RELATION_NAME VIEW_NAME
              FROM RDB$RELATIONS
...
export function previewRecordsQuery(tableName: string, limit: number): string {
  return `SELECT FIRST ${Math.abs(limit)} * FROM ${tableName};`;
}

export function countRecordsQuery(tableName: string): string {
